...
export default function Programmers() {
  ...
  const[featureId, setFeatueId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredProgrammer = programmersList.find(dev => dev.id === featureId)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featureId ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setFeatueId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
